...
function Req(s1, s2, s3, dis, z, bais) {
  useEffect(() => {
    setlsLoading(true);

    fetch(`https://z-score-api-1.onrender.com/?s1=${s1}&s2=${s2}&s3=${s3}&distric=${dis}&z=${z}&bais=${bais}`)
      .then((res) => res.json())
      .then(
        (result) => {
          setlsLoading(false);
  ...
  return (
    ...
    <TextInput placeholder='   z-score' style={{borderWidth:2,width:90}} value={zzz} onChangeText={setZed} keyboardType= 'phone-pad' />
      ...
      <Button title='calculate' onPress={() => Req(selected1, selected2, selected3, district, zzz, basi)} />
